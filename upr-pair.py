        ...
        while True:
            ...
            for context in all_contexts:
                ...
                if instruction.get("context_max_length", None) is not None:
                    ...
                    text_tokens = text_tokens[:instruction["context_max_length"]]
                    text = self.tokenizer.decode(text_tokens, skip_special_tokens=True)
                all_contexts_text.append(text)

            # run 2 for loops
            # create pairwise combinations of all_contexts
            for i, context in enumerate(all_contexts):
                text = all_contexts_text[i]
                for j, other_context in enumerate(all_contexts):
                    if i == j:
                        continue
                    other_text = all_contexts_text[j]
                    input_text = instruction["instruction"].format(
                        context=text, other_context=other_text, question=question
                    )
                    all_ids.append(input_text)
                    context_list.append((i, j))
            ...
            # iterated through all_outputs
            # now we need to get the scores for each context
            ctx_scores = {}
            for i, (ctx1, ctx2) in enumerate(context_list):
                option_logits = [all_outputs[i][-1][j].detach().float().cpu().item() for j in option_ids]
                probs = torch.softmax(torch.tensor(option_logits), dim=-1)
                # probs[0] is the probability of the first passage winning
                # probs[1] is the probability of the second passage winning
                # we aggregate the probabilities for each passage by summing them
                if ctx1 not in ctx_scores:
                    ctx_scores[ctx1] = 0
                if ctx2 not in ctx_scores:
                    ctx_scores[ctx2] = 0
                ctx_scores[ctx1] += probs[0].item()
                ctx_scores[ctx2] += probs[1].item()

            # now we have the scores for each context
            # we need to sort them and get the topk
            for i, ctx in enumerate(all_contexts):
                ctx["score"] = ctx_scores[i]
